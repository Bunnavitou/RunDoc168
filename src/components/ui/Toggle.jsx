export default function Toggle({ on, onToggle, disabled = false }) {
  return (
    <div
      ...
      className={`w-[38px] h-[22px] rounded-full relative flex-shrink-0 transition-colors duration-200 ${on ? 'bg-[#2563EB]' : 'bg-[#E3E5EA]'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
    >
      <div
